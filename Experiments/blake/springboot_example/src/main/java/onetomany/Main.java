...
class Main {
    ...
    @Bean
    CommandLineRunner initUser(UserRepository userRepository, LaptopRepository laptopRepository, PhoneRepository phoneRepository, PantryRepository pantryRepository) {
        return args -> {
            ...
            //testing code for pantry

            User user4 = new User("Bob", "bob@bobmail.com", new Date() );
            Pantry rice = new Pantry("rice");
            pantryRepository.save(rice); //need to save an object after you create it ig? caused a crash without
            user4.addPantries(rice);
